...
**Modeling improvements.** We considered pre-training then fine-tuning a dialog model that incorporates both human and bot utterances, but chose the more straightforward BERT fine-tuning approach as we assumed human utterances were sufficient to solve this task. In retrospect, our assumption was too strong, as the BERT fine-tuning approach is inherently unable to contextualize the user’s utterances with respect to the bot’s prompts and questions. This can produce age-discriminative features, as users of different age groups may respond to particular questions or prompts in age-specific ways (i.e. perhaps children are more likely to give short telegraphic responses). A dialog model paradigm would allow us to sidestep this limitation.
